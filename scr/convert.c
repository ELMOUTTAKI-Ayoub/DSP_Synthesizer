...
int convertEvent (trackChunk* sample, const uint8_t* index, uint32_t* deltaTime, uint32_t* events){
	...
	while(index[offset] & 0b10000000){
		_deltaTime += index[offset++] & 0b01111111;
	}
	_deltaTime += index[offset++] & 0b01111111;
    //Update delta Time
	...
	switch(event){
		case NOTE_ON :{
			printf("\tNote On \n");
            (*events)++;
            command  com = {NOTE_ON, index[offset]&0x0F, _deltaTime, index[offset+1],index[offset+2]};
            //void printCommand(uint16_t command, uint16_t channel, uint16_t deltaTime, uint16_t noteNumber);
            if(index[offset+2]==0x00){
                //Doch Note aus
                com.command = NOTE_OFF;
            }
            printCommand(com);
            (*deltaTime)=0;
		...
		case NOTE_OFF :{
            printf("\tNote Off \n");
            (*events)++;
            command  com = {NOTE_OFF, index[offset]&0x0F, _deltaTime, index[offset+1],index[offset+2]};
            printCommand(com);
            offset+=3;
			noteOff++;
			break;
...
void printCommand(command com){
    ...
    if ( fd != NULL ){
        fprintf(fd,"    {0x%02X, 0x%02X, 0x%02X, 0x%02X, 0x%02X},\n",
            com.command,
            com.channel,
            com.deltaTime,
            com.noteNumber,
...
void printChunkList(midiTracks* tracks){
    ...
    if ( fd != NULL ){
        ...
        for(; index<tracks->numTracks-1; index++){
            fprintf(fd,"    {0x%02X, .size = {.size = 0x%04X}, 0x%02X, 0x%02X, 0x%02X},\n",
                0x00,
                tracks->tracks[index].size.size,
                0x00,
                0x00,
                0x00);
        }
...
void printMidi(uint8_t trackCount){
    ...
    if ( fd != NULL ){
        fprintf(fd,"midiTracks midi = { %d , trackList, commandList};\n",trackCount);
        fclose(fd);
    }
}
